...
//    }
    ...
    public function update(array $attributes, $id)
    {
        if (array_key_exists("start_date",$attributes)){
            ...
            if ($attributes['start_date'] != $driver->start_date){
                // lấy ra năm tháng cho start_date, trường hợp nếu start date null thì tìm theo closing date
                $getMonthYear = Carbon::parse($attributes['start_date'] == '' ? $attributes['start_date'] : $driver->created_at)->format('Y-m');
                // check final_closing
        ...
        $data = parent::update($attributes, $id);
