...
class CashInDeleteTest extends TestCase
{
    /**
    ...
    public function testCashInDeleteSuccess()
    {
        ...
        $this->actingAs($user)->json('delete', 'api/cash-in'."/$cashIn->id", [
            'token' => $token,
        ])->assertStatus(CODE_SUCCESS);
    }

    /////////Delete Start//////////
    public function testCashInDeleteIdNotFound()
    {
        $dayForCourse6 = Carbon::now()->addDay()->format("Y-m-d");
        $course = Course::create([
            "customer_id"=> 5,
        ...
        $user = User::where('user_code', '=', '1122')->first();
        $token = \JWTAuth::fromUser($user);
        $this->actingAs($user)->json('delete', 'api/cash-in'."/$cashInNotfound", [
            'token' => $token,
