...
class CourseDetailTest extends TestCase
{
    ...
    public function testCourseDetailSuccess()
    {
        ...
        $this->actingAs($user)->json('get', "api/course/$course->id", [
            'token' => $token,
        ])->assertStatus(CODE_SUCCESS);
    }

    /////////Detail Start//////////
    public function testCourseDetailNotFound()
    {
        $user = User::where('user_code', '=', '1122')->first();
        $token = \JWTAuth::fromUser($user);
        $response = $this->actingAs($user)->json('get', "api/course/99", [
            'token' => $token,
