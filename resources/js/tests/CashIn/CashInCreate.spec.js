...
import CashInCreate from '@/pages/CashManagement/CashReceipt/create';
import { mount, createLocalVue } from '@vue/test-utils';

describe('TEST COMPONENT CASH-IN CREATE', () => {
	test('Check render page', () => {
		const localVue = createLocalVue();
		const wrapper = mount(CashInCreate, {
			localVue,
			store,
			router,
		});

		const PAGE = wrapper.find('.page-cash-create');
	...
	test('Check render title page', () => {
		...
		expect(TITLE.text()).toEqual('LIST_CASH.TITLE_CASH_CREATE');
